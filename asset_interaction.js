...
function createAsset()
{
    ...
    $('#loaderMessages').html('<u>Entrain de créer le certificat</u><br /><br /><span id="messages"></i>en attente...</i></span><br /></div>&nbsp;');
    ...
    xhr.onreadystatechange = function (){
        if(xhr.readyState === 4)
		{
            ...
            else
			{
                ...
                $('#chooseConfHd').html('<span>Creation Terminée</span>');
                $('#confTxt').html('Certificat Crée: '+ JSON.parse(array[array.length - 1]).v5cID);
            }
        }
    };
        ...
        if(xhr.readyState === 4)
		{
            ...
            if(increment == bigData.length - 1){
                if(errArr.length == 0)
				{
                    ...
                    $('#loader img').hide();
                    $('#loaderMessages').append('<br /><br /><span id="okTransaction" onclick="confTrans();">OK</span>');
                    $('#confTxt').html('Mise à jour inscrite dans la chaîne de bloc.');
                }
                else
				{
                    $('#latestSpan'+(increment+1)).html('&nbsp;&#10004');
