	...
	*/
	...
	xhr.onreadystatechange = function (){
		if(xhr.readyState === 4)
		{
			...
			{
				var data = d[i];
				if(data.VIN == 0) data.VIN = '&lt;<i>Grammes</i>&gt;';
				if(data.make.toLowerCase() == 'undefined' || data.make.trim() == '') data.make = '&lt;<i>Type</i>&gt;';
				if(data.model.toLowerCase() == 'undefined' || data.model.trim() == '') data.model = '&lt;<i>Etat physique</i>&gt;';
				if(data.reg.toLowerCase() == 'undefined' || data.reg.trim() == '') data.reg = '&lt;<i>Région</i>&gt;';
				if(data.colour.toLowerCase() == 'undefined' || data.colour.trim() == '') data.colour = '&lt;<i>Pays</i>&gt;';
